...
from social_extract.visual_description import OpenaiVisualDescriber, _coerce_description
...
class StubOpenaiClient:
    ...
    @property
    def chat(self) -> StubOpenaiClient:
        return self
    ...
    def create(self, *, model: str, messages: list[dict], **kwargs) -> SimpleNamespace:
        self.create_calls.append({"model": model, "messages": messages, "kwargs": kwargs})
...
def test_openai_visual_describer_uses_keyframe_file_and_ocr_context(tmp_path: Path) -> None:
    ...
    describer = OpenaiVisualDescriber(client=client, model="openai-test")
    ...
    assert page["visual_provider"] == "openai"
    assert page["visual_model"] == "openai-test"
    assert page["visual_cache_hit"] is False
    assert result.meta["described_pages"] == 1
    assert client.create_calls[0]["model"] == "openai-test"
    assert "OCR text" in client.create_calls[0]["messages"][0]["content"][0]["text"]


def test_openai_visual_describer_reuses_cache_for_same_frame(tmp_path: Path) -> None:
    ...
    describer = OpenaiVisualDescriber(client=client)
...
def test_openai_visual_describer_respects_page_limit(tmp_path: Path) -> None:
    ...
    result = OpenaiVisualDescriber(client=client).describe_pages(
...
def test_openai_visual_describer_optional_failure_keeps_page(tmp_path: Path) -> None:
    frame_path = _write_frame(tmp_path, "frames/page_0000.jpg", b"frame")
    client = StubOpenaiClient(["__raise__"])
...
def test_openai_visual_describer_required_failure_raises(tmp_path: Path) -> None:
    frame_path = _write_frame(tmp_path, "frames/page_0000.jpg", b"frame")
    client = StubOpenaiClient(["__raise__"])

    with pytest.raises(Exception, match="Visual description failed"):
        OpenaiVisualDescriber(client=client).describe_pages(
            [_page(frame_path, "OCR")],
            tmp_path,
